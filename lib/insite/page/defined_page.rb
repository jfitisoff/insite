class DefinedPage
    ...
    def set_attributes(*args)
      ...
      args.each do |arg|
        ...
        when :page_template
          ...
          raise Insite::Errors::PageConfigError, "Unsupported page attribute argument: #{arg} for #{self} page definition. Argument class: #{arg.class}. Arguments must be one or more of the following symbols: :navigation_disabled, :template."
        end
      end
    ...
    if @required_arguments.present? && !args
      @required_arguments.each do |arg|
        if @site.respond_to?(arg)
          ...
          raise Insite::Errors::PageInitError, "No arguments provided when attempting to initialize #{self.class.name}. This page object requires the following arguments for initialization: :#{@required_arguments.join(', :')}.\n\n#{caller.join("\n")}"
        end
      end
      ...
      @required_arguments.each do |arg| # Try to extract each URL argument from the hash or object provided, OR from the site object.
        if args.is_a?(Hash) && args.present?
          ...
          if args[arg] #The hash has the required argument.
            ...
            raise Insite::Errors::PageInitError, "A required page argument is missing. #{args.class} was provided, but this object did not respond to :#{arg}, which is necessary to build an URL for the #{self.class.name} page.\n\n#{caller.join("\n")}"
          end
      ...
      raise Insite::Errors::PageInitError, "#{args.class} was provided as a #{self.class.name} initialization argument, but the page URL doesn't require any arguments.\n\n#{caller.join("\n")}"
    else
      # Do nothing here yet.
    end
  ...
  # Navigates to the page that it's called on. Raises a SiteObject::Insite::Errors::PageNavigationNotAllowedError when
  # navigation has been disabled for the page. Raises a SiteObject::Insite::Errors::WrongPageError if the
  # specified page isn't getting displayed after navigation.
  def visit
    if navigation_disabled?
      raise Insite::Errors::PageNavigationNotAllowedError, "Navigation has been disabled for the #{self.class.name} page. This was done when defining the page class and usually means that the page can't be reached directly through a URL and requires some additional work to access."
    end

    @browser.goto(@url)

    if @url_matcher
      raise Insite::Errors::WrongPageError, "Navigation check failed after attempting to access the #{self.class.name} page. Current URL #{@browser.url} did not match #{@url_template.pattern}. A URL matcher was also defined for the page and the secondary check against the URL matcher also failed. URL matcher: #{@url_matcher}" unless on_page?
    else
      raise Insite::Errors::WrongPageError, "Navigation check failed after attempting to access the #{self.class.name} page. Current URL #{@browser.url} did not match #{@url_template.pattern}" unless on_page?
    end
